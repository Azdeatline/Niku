    ...
    if user_id in admin_id:
        ...
        if len(command) > 2:
            ...
            if period in subscription_periods:
                ...
                response = "Invalid subscription period. Use: 1min, 1hour, 6hours, 12hours, 1day, 3days, 7days, 15days, 1month, or 2months."
        else:
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/c/2443310728/41"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['7X'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 7X \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑  \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 7X MODS.
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
